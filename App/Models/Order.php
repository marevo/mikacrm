...
//6) sum(select sumPayment from payments) 7) можно ли менять стоимость комплектующих, если удален (isTrash=1) - то не показываем, а если не удален isTrash=0 тогда показываем
        public static function selectForView( ){
            ...
            $queryNew= "SELECT ttt.idOrder AS idOrder, ttt.dateBegin AS dateBegin, ttt.dateEnd AS dateEnd, ttt.orderName AS `name`, ttt.nameClient AS nameClient, ttt.orderPrice, ttt.isReady, ttt.isCompleted, SUM(sumPayment) AS sumPayment FROM 
       (SELECT  o.id AS idOrder , o.dateOfOrdering AS dateBegin, o.dateOfComplation AS dateEnd, o.name AS orderName, c.name AS nameClient, o.orderPrice AS orderPrice, o.isReady AS isReady , o.isCompleted AS isCompleted 
        FROM orders o, clients c
        WHERE o.isTrash != 1  AND o.idClient = c.id)
        AS ttt
    LEFT OUTER JOIN payments AS p ON ttt.idOrder = p.idOrder
    GROUP BY idOrder
...
//** найти все удаленные заказы (в корзине) */
    ...
    {
        $queryFindTrashedOrders = "SELECT ttt.idOrder AS idOrder, ttt.dateBegin AS dateBegin, ttt.dateEnd AS dateEnd, ttt.orderName AS `name`, ttt.nameClient AS nameClient, ttt.orderPrice, ttt.isReady, ttt.isCompleted, SUM(sumPayment) AS sumPayment FROM 
       (SELECT  o.id AS idOrder , o.dateOfOrdering AS dateBegin, o.dateOfComplation AS dateEnd, o.name AS orderName, c.name AS nameClient, o.orderPrice AS orderPrice, o.isReady AS isReady , o.isCompleted AS isCompleted 
        FROM orders o, clients c
        WHERE o.isTrash = 1  AND o.idClient = c.id)
        AS ttt
    ...
    GROUP BY idOrder
    ORDER BY dateBegin DESC ;";
        $queryFindTrashedOrders_Old= "SELECT  o.id AS idOrder ,o.dateOfOrdering AS dateBegin,
                                  o.dateOfComplation AS dateEnd, o.name, c.name AS nameClient ,
...
public static function getOrdersLikeNameClient( $likeName){
    ...
    $queryFindOrderLikeName_Old = "SELECT o.id AS idOrder , o.dateOfOrdering AS dateBegin, o.dateOfComplation AS dateEnd,
    ...
    $queryFindOrderLikeName= "SELECT ttt.idOrder AS idOrder, ttt.dateBegin AS dateBegin, ttt.dateEnd AS dateEnd, ttt.orderName AS `name`, ttt.nameClient AS nameClient, ttt.orderPrice, ttt.isReady, ttt.isCompleted, SUM(sumPayment) AS sumPayment FROM 
       (SELECT  o.id AS idOrder , o.dateOfOrdering AS dateBegin, o.dateOfComplation AS dateEnd, o.name AS orderName, c.name AS nameClient, o.orderPrice AS orderPrice, o.isReady AS isReady , o.isCompleted AS isCompleted 
        FROM orders o, clients c
        WHERE c.name  LIKE '%$likeName%'  AND o.idClient = c.id )
        AS ttt
    LEFT OUTER JOIN payments AS p ON ttt.idOrder = p.idOrder
    GROUP BY idOrder
    ORDER BY dateBegin DESC;
...
//            var_dump('<br>последняя строка в результата нет !!! function query in Db.php<br>');
    ...
    public static function getOrdersLikeName( $likeName){
       ...
       (SELECT  o.id AS idOrder , o.dateOfOrdering AS dateBegin, o.dateOfComplation AS dateEnd, o.name AS orderName, c.name AS nameClient, o.orderPrice AS orderPrice, o.isReady AS isReady , o.isCompleted AS isCompleted 
        FROM orders o, clients c
        WHERE o.name  LIKE '%$likeName%'  AND o.idClient = c.id )
        AS ttt
    LEFT OUTER JOIN payments AS p ON ttt.idOrder = p.idOrder
    GROUP BY idOrder
    ORDER BY dateBegin DESC;
...
//        var_dump($res);
    ...
    //найти название клиента для этого заказа
