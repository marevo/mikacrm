...
$('.rowSearch').ready().on('click',function () {
    ...
    if(target.name == 'searchForNameClient'){
        ...
        if(nameClient != '' && nameClient.length >2){
            jquery_send('.divForAnswerServer','post','../templates/controllerViewAllOrders.php',
...
//     }
// }
//функция загрузки данных одного заказа в окошко '#main_modul'
//функция подтяжки в id='#main_modul' viewOneOrder.js с заданным id 
function includeViewOneOrder(tarHref,idOneOrder) {
    jquery_send('#main_modul','get','../templates/controllerViewAllOrders.php',
        ['includeViewOneOrder','tarHref','id'],
        ['',tarHref, idOneOrder]);
